...
def MLP(NR, ER, Kr, UAr, SE, CH, output, not_trained):
#Random Forest
...
#input single electron data
        SE_data = pd.read_csv(SE, sep = " ", header = None )
        SE_data.drop([60,61], axis = 1, inplace = True)
        SE_data.columns=["event_id" if x == 1 else x for x in range(60)]
        target_3 = [2 for x in range(len(SE_data))]
        SE_data['target'] = target_3
        name_3 = ['SE' for x in range(len(SE_data))]
        SE_data['name'] = name_3

#input cherenkov data
        CH_data = pd.read_csv(CH, sep = " ", header = None )
        CH_data.drop([60,61], axis = 1, inplace = True)
        CH_data.columns=["event_id" if x == 1 else x for x in range(60)]
        target_4 = [3 for x in range(len(CH_data))]
        CH_data['target'] = target_4
        name_4 = ['SE' for x in range(len(CH_data))]
        CH_data['name'] = name_4        
        
    #concatenate data together
        frames = [NR_data, ER_data, Kr_data, SE_data, CH_data]
    ...
    #   clf = tree.DecisionTreeClassifier(random_state=1)
        ...
        clf = MLPClassifier(activation='logistic', random_state=1) #hidden_layer_sizes=(13,13,13),max_iter=500, 
    ...
    #predict
      ...
      #proba
        ...
        #plt.show()
        #fig5.savefig("proba.pdf", bbox_inches = 'tight')
    ...
    #proba = clf_trained.predict_proba(UAr_test_data_0)
    #fig5 = plt.figure()
    #plt.hist(proba, bins=1500, density=True, log=True) #density=True
    #plt.show()
    #fig5.savefig("proba.pdf", bbox_inches = 'tight')
...
if __name__ == "__main__":
    ...
    CH = sys.argv[6]
    output = sys.argv[7]
    not_trained = sys.argv[8]
    MLP(NR, ER, Kr, UAr, SE, CH, output, not_trained)
